...
const getPortfolioAssets = async (portfolio_id) => {
  try {
    ...
    portfolioShares.forEach((share) => {
      const cost =
        (share.price + share.price * share.commission) * share.quantity;
...
const getGroupedPortfolioShares = async (portfolio_id) => {
  try {
    const { portfolioShares } = await getPortfolioAssets(portfolio_id);
    ...
    for (const category in groupedShares) {
      if (groupedShares.hasOwnProperty(category)) {
        groupedShares[category].totalPrice = parseFloat(
          groupedShares[category].totalPrice.toFixed(2),
        );
      }
    }
    ...
    return { error: 'Failed to fetch grouped portfolio shares' };
  }
};
